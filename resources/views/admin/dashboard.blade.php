...
@section('content')
    <div class="min-h-screen bg-gradient-to-r from-[#0a7558] to-[#042f2e] font-quicksand text-black flex overflow-hidden">
        ...
        <aside
            ...
            <nav class="flex-1 px-6 space-y-2 overflow-y-auto">
                ...
                <button onclick="showTab('campaigns')" id="btn-campaigns"
                    class="admin-nav-btn flex items-center gap-4 w-full px-6 py-4 rounded-xl transition-all duration-200 text-white/70">
                ...
                <button onclick="showTab('users')" id="btn-users"
                    class="admin-nav-btn flex items-center gap-4 w-full px-6 py-4 rounded-xl transition-all duration-200 text-white/70">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                ...
                <button onclick="showTab('partners')" id="btn-partners"
                    class="admin-nav-btn flex items-center gap-4 w-full px-6 py-4 rounded-xl transition-all duration-200 text-white/70">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                        <path
                ...
                <button onclick="showTab('categories')" id="btn-categories"
                    class="admin-nav-btn flex items-center gap-4 w-full px-6 py-4 rounded-xl transition-all duration-200 text-white/70">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                        <path
            ...
            <div class="p-6 border-t border-[#996515]/30">
                <button onclick="ApiClient.logout()"
                    class="flex items-center justify-center gap-3 w-full py-4 rounded-xl transition-all duration-200 text-white font-bold hover:text-[#996515]">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
        ...
        <main class="ml-80 flex-1 p-12 lg:p-16 overflow-y-auto h-screen relative z-10">
            ...
            <header class="flex items-center justify-between mb-16">
                <div>
                    <h1 class="text-md font-black uppercase tracking-[0.5em] text-white/60 mb-2">welcome back Our Admin</h1>
                </div>
            </header>
                                ...
                                @foreach ($campaigns as $c)
                                    <tr class="hover:bg-[#064e3b]/30 transition-colors" id="campaign-{{ $c->id }}">
                                        <td class="px-8 py-6">
                                            ...
                                            <div class="text-xs text-white/40 mt-1">ID: {{ substr($c->id, 0, 8) }}</div>
                                        </td>
                                        <td class="px-8 py-6">
                                            <div class="font-bold text-xl text-[#996515]">
                                                ${{ number_format($c->target_amount) }}</div>
                                        </td>
                                        <td class="px-8 py-6">
                                            <span class="text-sm font-bold text-white">{{ strtoupper($c->status) }}</span>
                                        </td>
                                ...
                                @foreach ($users as $u)
                                    <tr class="hover:bg-[#064e3b]/30 transition-colors">
                                        ...
                                        <td class="px-8 py-6 text-right">
                                            <button onclick="banUser({{ $u->id }}, {{ $u->is_banned ? 'false' : 'true' }})"
                                                class="admin-btn">{{ $u->is_banned ? 'Unban' : 'Ban' }}</button>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                            ...
                            <tbody class="divide-y divide-[#996515]/10">
                                @foreach ($organisations as $o)
                                    <tr class="hover:bg-[#064e3b]/30 transition-colors" id="org-{{ $o->id }}">
                                        ...
                                        <td class="px-8 py-6">
                                            <span class="text-sm font-bold text-white">{{ $o->is_verified ? 'VERIFIED' : 'PENDING' }}</span>
                                        </td>
                                        <td class="px-8 py-6 text-right" id="org-row-{{ $o->id }}">
                                            <div class="flex items-center justify-end gap-2">
                                                @if (!$o->is_verified)
                                                    <button onclick="verifyOrg({{ $o->id }})"
                                                        class="admin-btn">Verify</button>
                                                @else
                                                    <button onclick="rejectOrg({{ $o->id }})"
                                                        class="admin-btn">Revoke</button>
                                                @endif
                                            </div>
                                        </td>
                                    </tr>
                            ...
                            <form action="#" class="space-y-6">
                                <div class="space-y-3">
                                    <label class="text-xs font-black text-[#996515] uppercase tracking-wider">Category
                                        Name</label>
                                    <input type="text" placeholder="e.g. Education"
                                ...
                                <button onclick="createCategory(event)"
                                    class="admin-btn w-full py-4">Create Category</button>
                            </form>
                        </div>
                            ...
                            <table class="w-full text-left">
                                ...
                                <tbody class="divide-y divide-[#996515]/10">
                                    @foreach ($categories as $cat)
                                        <tr class="hover:bg-[#064e3b]/30 transition-colors">
                                            <td class="px-8 py-6 font-bold text-base text-white">
    ...
    <style>
        .admin-btn {
            @apply px-4 py-2 bg-black text-white text-xs font-bold rounded-lg hover:bg-gray-800 transition-all uppercase tracking-wider;
        }
    </style>
    ...
    <script>
        ...
        function showTab(tabName) {
            ...
            document.querySelectorAll('.admin-nav-btn').forEach(btn => {
                btn.className =
                    'admin-nav-btn flex items-center gap-4 w-full px-6 py-4 rounded-xl transition-all duration-200 text-white/70';
            });

            document.getElementById('btn-' + tabName).className =
                'admin-nav-btn flex items-center gap-4 w-full px-6 py-4 rounded-xl transition-all duration-200 text-[#996515] bg-[#996515]/10 border-l-4 border-[#996515]';
        }
        ...
        async function approveCampaign(id) {
            try {
                ...
                if (response.status === 'success') {
                    ...
                    const actionsCell = row.querySelector('td:nth-child(4) div');
                    actionsCell.innerHTML = `<button onclick="rejectCampaign(${id})" class="admin-btn">Reject</button>`;
                }
            } catch (error) {
        ...
        async function verifyOrg(id) {
            try {
                ...
                if (response.status === 'success') {
                    ...
                    const actionsCell = row.querySelector('td:nth-child(3) div');
                    actionsCell.innerHTML = `<button onclick="rejectOrg(${id})" class="admin-btn">Revoke</button>`;
                }
            } catch (error) {
        ...
        async function rejectOrg(id) {
            ...
            try {
                ...
                if (response.status === 'success') {
                    ...
                    const actionsCell = row.querySelector('td:nth-child(3) div');
                    actionsCell.innerHTML = `<button onclick="verifyOrg(${id})" class="admin-btn">Verify</button>`;
                }
            } catch (error) {
        ...
        async function banUser(id, shouldBan) {
            ...
            try {
                const response = await ApiClient.post(`/api/users/${id}/${action}`);
        ...
        async function createCategory(event) {
            ...
            try {
                const response = await ApiClient.post('/api/categories', { category_name: categoryName });
                if (response.status === 'success') {
                    input.value = '';
                    const tbody = document.querySelector('#tab-categories tbody');
                    const newRow = document.createElement('tr');
                    newRow.className = 'hover:bg-[#064e3b]/30 transition-colors';
                    newRow.innerHTML = `<td class="px-8 py-6 font-bold text-base text-white">${categoryName}</td>`;
