...
@section('content')
    <div class="min-h-screen bg-gradient-to-r from-[#0a7558] to-[#042f2e] font-quicksand text-black flex overflow-hidden">
        ...
        {{-- Sidebar --}}
        <aside class="w-80 bg-gradient-to-l from-[#0a7558] to-[#042f2e] border-r-2 border-[#996515] flex flex-col h-screen fixed top-0 left-0 z-50 shadow-2xl">
            {{-- Brand --}}
            ...
            <nav class="flex-1 px-6 space-y-2 overflow-y-auto">
                <button onclick="switchTab('campaigns')" data-tab="campaigns" class="sidebar-nav-item w-full flex items-center gap-4 px-6 py-4 rounded-xl text-white/70 hover:text-[#996515] transition-all duration-200">
                ...
                <button onclick="switchTab('users')" data-tab="users" class="sidebar-nav-item w-full flex items-center gap-4 px-6 py-4 rounded-xl text-white/70 hover:text-[#996515] transition-all duration-200">
                ...
                <button onclick="switchTab('partners')" data-tab="partners" class="sidebar-nav-item w-full flex items-center gap-4 px-6 py-4 rounded-xl text-white/70 hover:text-[#996515] transition-all duration-200">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                ...
                <button onclick="switchTab('categories')" data-tab="categories" class="sidebar-nav-item w-full flex items-center gap-4 px-6 py-4 rounded-xl text-white/70 hover:text-[#996515] transition-all duration-200">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                        <path d="M7 7h.01M7 3h5c.5 0 1 .2 1.4.6l7 7a2 2 0 010 2.8l-7 7a2 2 0 01-2.8 0l-7-7A2 2 0 013 12V7a4 4 0 014-4z" />
                    </svg>
            ...
            <div class="p-6 border-t border-[#996515]/30">
                <button onclick="ApiClient.logout()" class="w-full flex items-center justify-center gap-3 py-4 rounded-xl text-white font-bold hover:text-[#996515] transition-all duration-200">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
        ...
        <main class="ml-80 flex-1 p-12 lg:p-16 overflow-y-auto h-screen relative z-10">
            ...
            <div class="space-y-16">
                ...
                <section id="tab-campaigns" class="admin-tab space-y-10">
                    <div class="border border-[#996515] rounded-2xl overflow-hidden bg-[#042f2e]/70 shadow-2xl shadow-[#996515]/20">
                                ...
                                @foreach ($campaigns as $c)
                                    <tr class="hover:bg-[#064e3b]/30 transition-colors" id="campaign-{{ $c->id }}">
                                        <td class="px-8 py-6">
                                            ...
                                            <div class="text-xs text-white/40 mt-1">{{ $c->created_at->format('M d, Y') }}</div>
                                        </td>
                                        <td class="px-8 py-6">
                                        ...
                                        <td class="px-8 py-6">
                                            <div class="text-xs text-white/60">{{ $c->category->category_name ?? 'N/A' }}</div>
                                        </td>
                                        <td class="px-8 py-6">
                                            <div class="font-bold text-lg text-[#996515]">${{ number_format($c->target_amount) }}</div>
                                        </td>
                                        <td class="px-8 py-6">
                                            <div class="font-bold text-lg text-emerald-400">${{ number_format($c->current_amount) }}</div>
                                            <div class="text-xs text-white/40">{{ $c->target_amount > 0 ? round(($c->current_amount / $c->target_amount) * 100, 1) : 0 }}%</div>
                                        </td>
                                        <td class="px-8 py-6">
                                            <span class="text-sm font-bold text-white campaign-status">{{ strtoupper($c->status) }}</span>
                                        </td>
                                        <td class="px-8 py-6 text-right">
                                            <div class="flex items-center justify-end gap-2 campaign-actions">
                                                @if ($c->status !== 'active')
                                                    <button onclick="approveCampaign('{{ $c->id }}')" class="px-4 py-2 bg-black text-white text-xs font-bold rounded-lg hover:bg-gray-800 transition-all uppercase tracking-wider">Approve</button>
                                                @endif
                                                <button onclick="rejectCampaign('{{ $c->id }}')" class="px-4 py-2 bg-black text-white text-xs font-bold rounded-lg hover:bg-gray-800 transition-all uppercase tracking-wider">Reject</button>
                                            </div>
                                        </td>
                                    </tr>
                                @endforeach
                            ...
                            <tbody class="divide-y divide-[#996515]/10">
                                @foreach ($users as $u)
                                    <tr class="hover:bg-[#064e3b]/30 transition-colors" id="user-{{ $u->id }}">
                                        <td class="px-8 py-6 font-bold text-base text-white">{{ $u->first_name }} {{ $u->last_name }}</td>
                                        <td class="px-8 py-6 text-sm text-white/60">{{ $u->email }}</td>
                                        <td class="px-8 py-6">
                                            <span class="text-sm font-bold text-white">{{ strtoupper($u->role) }}</span>
                                        </td>
                                        <td class="px-8 py-6">
                                            <span class="text-xs font-bold {{ $u->is_banned ? 'text-red-400' : 'text-emerald-400' }}">{{ $u->is_banned ? 'BANNED' : 'ACTIVE' }}</span>
                                        </td>
                                        <td class="px-8 py-6 text-right">
                                            <button onclick="banUser({{ $u->id }}, {{ $u->is_banned ? 'false' : 'true' }})" class="px-4 py-2 bg-black text-white text-xs font-bold rounded-lg hover:bg-gray-800 transition-all uppercase tracking-wider user-ban-btn">
                                    ...
                                    <tr class="hover:bg-[#064e3b]/30 transition-colors" id="org-{{ $o->id }}">
                                        ...
                                        <td class="px-8 py-6 text-right">
                                            <div class="flex items-center justify-end gap-2 org-actions">
                                                @if (!$o->is_verified)
                                                    <button onclick="verifyOrg({{ $o->id }})" class="px-4 py-2 bg-black text-white text-xs font-bold rounded-lg hover:bg-gray-800 transition-all uppercase tracking-wider">Verify</button>
                                                @else
                                                    <button onclick="rejectOrg({{ $o->id }})" class="px-4 py-2 bg-black text-white text-xs font-bold rounded-lg hover:bg-gray-800 transition-all uppercase tracking-wider">Revoke</button>
                                                @endif
                                            </div>
                                        </td>
                                    </tr>
                ...
                <section id="tab-categories" class="admin-tab hidden space-y-10">
                    <div class="grid grid-cols-1 xl:grid-cols-12 gap-8">
                        <div class="xl:col-span-4 bg-[#042f2e]/70 p-10 rounded-2xl border border-[#996515] shadow-2xl shadow-[#996515]/20">
                            <div class="mb-8">
                            ...
                            <div class="space-y-6">
                                <div class="space-y-3">
                                    ...
                                    <input type="text" id="new-category-name" placeholder="e.g. Education" class="w-full bg-[#064e3b]/30 border-2 border-[#996515]/30 rounded-xl px-6 py-4 text-sm font-bold outline-none focus:border-[#996515] transition-all placeholder:text-white/30 text-white">
                                </div>
                                <button onclick="createCategory()" id="create-category-btn" class="px-4 py-4 bg-black text-white text-xs font-bold rounded-lg hover:bg-gray-800 transition-all uppercase tracking-wider w-full">Create Category</button>
                                <div id="category-message" class="hidden text-xs font-bold"></div>
                            </div>
                        </div>
                        <div class="xl:col-span-8 border border-[#996515] rounded-2xl overflow-hidden bg-[#042f2e]/70 shadow-2xl shadow-[#996515]/20">
                            <table class="w-full text-left">
                                <thead class="bg-[#064e3b]/50 border-b-2 border-[#996515]/20">
                                    <tr class="text-xs font-black uppercase tracking-[0.2em] text-[#996515]">
                                ...
                                <tbody class="divide-y divide-[#996515]/10" id="categories-tbody">
                                    @foreach ($categories as $cat)
                                        <tr class="hover:bg-[#064e3b]/30 transition-colors" id="category-{{ $cat->id }}">
                                            <td class="px-8 py-6 font-bold text-base text-white">{{ $cat->category_name }}</td>
                                            <td class="px-8 py-6 text-right">
                                                <button onclick="deleteCategory({{ $cat->id }})" class="px-4 py-2 bg-red-900 text-white text-xs font-bold rounded-lg hover:bg-red-800 transition-all uppercase tracking-wider">Delete</button>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
    ...
    <script>
        ...
        async function createCategory() {
            ...
            try {
                const response = await ApiClient.post('/categories', { category_name: categoryName });
        ...
        async function approveCampaign(id) {
            try {
                ...
                if (response.status === 'success') {
                    ...
                    row.querySelector('.campaign-actions').innerHTML = `<button onclick="rejectCampaign('${id}')" class="px-4 py-2 bg-black text-white text-xs font-bold rounded-lg hover:bg-gray-800 transition-all uppercase tracking-wider">Reject</button>`;
                }
            } catch (error) {
        ...
        async function verifyOrg(id) {
            try {
                ...
                if (response.status === 'success') {
                    ...
                    row.querySelector('.org-actions').innerHTML = `<button onclick="rejectOrg(${id})" class="px-4 py-2 bg-black text-white text-xs font-bold rounded-lg hover:bg-gray-800 transition-all uppercase tracking-wider">Revoke</button>`;
                }
            } catch (error) {
        ...
        async function rejectOrg(id) {
            try {
                ...
                if (response.status === 'success') {
                    ...
                    row.querySelector('.org-actions').innerHTML = `<button onclick="verifyOrg(${id})" class="px-4 py-2 bg-black text-white text-xs font-bold rounded-lg hover:bg-gray-800 transition-all uppercase tracking-wider">Verify</button>`;
                }
            } catch (error) {
