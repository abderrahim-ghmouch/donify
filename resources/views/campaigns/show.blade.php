...
@section('content')
@php
    $leadName = $campaign->organisation?->name
    ...
    $leadImage = $campaign->organisation?->verificationDocument?->url
        ?? $campaign->user?->images?->url
...
<div class="min-h-screen bg-[#fbf8f6] font-quicksand relative overflow-hidden">
    ...
    <div class="absolute inset-x-0 bottom-0 h-full bg-gradient-to-t from-[#064e3b]/25 via-[#064e3b]/5 to-transparent pointer-events-none z-0"></div>
    ...
    <section class="relative pt-24 md:pt-32 pb-10 md:pb-14 px-5 md:px-8 z-10">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="flex flex-wrap items-center gap-4 md:gap-6 mb-10 md:mb-12">
                <a href="{{ route('campaigns.index') }}" class="group inline-flex items-center gap-3 rounded-md border border-emerald-800/25 bg-white/80 px-5 py-3 text-xs font-black uppercase tracking-[0.22em] text-[#064e3b] shadow-sm transition-all hover:border-emerald-700/50 hover:bg-white hover:text-[#1A1A1A]">
                    <span class="text-base leading-none transform group-hover:-translate-x-1 transition-transform">←</span> Campaigns
                </a>
                ...
                <div class="w-full sm:w-auto sm:ml-auto flex items-center gap-3">
                    ...
                    <div class="px-4 md:px-5 py-2 rounded-md bg-[#1A1A1A] text-[#059669] text-[9px] font-black uppercase tracking-widest border border-[#059669]/60 shadow-sm">
            ...
            <h1 class="text-4xl md:text-6xl font-black text-[#1A1A1A] leading-[1.08] mb-10 md:mb-12 tracking-tight max-w-5xl">
                {{ $campaign->title }}.
            </h1>
    ...
    <section class="relative z-10 px-5 md:px-8 pb-24 md:pb-32">
        <div class="max-w-7xl mx-auto flex flex-col lg:flex-row gap-10 xl:gap-16">
            ...
            <div class="flex-1 space-y-8 md:space-y-10">
                ...
                {{-- Intent & Purpose --}}
                <div class="bg-[#fff7ed]/85 backdrop-blur-xl p-7 md:p-10 rounded-lg border border-amber-300/80 shadow-sm">
                    <div class="flex items-center gap-4 mb-8 md:mb-10">
                        <div class="h-px flex-grow bg-amber-500/20"></div>
                        <h2 class="text-[10px] md:text-[11px] font-black text-[#1A1A1A] tracking-[0.24em] md:tracking-[0.4em] uppercase text-center">The Mission Narrative</h2>
                        <div class="h-px flex-grow bg-amber-500/20"></div>
                    </div>
                    <div class="text-[#1A1A1A] font-medium leading-8 whitespace-pre-line text-base md:text-lg opacity-90 tracking-tight">
            ...
            <div class="w-full lg:w-[420px] xl:w-[440px] shrink-0">
                <div class="lg:sticky lg:top-28">
                    <div class="bg-gradient-to-br from-[#064e3b] to-[#042f24] p-7 md:p-10 rounded-lg shadow-md border border-emerald-300/35 relative overflow-hidden">
                        {{-- Light Burst Decor --}}
                        <div class="absolute -top-32 -right-32 w-64 h-64 bg-emerald-400/10 rounded-full blur-[100px]"></div>

                        <div class="relative z-10">
                            <h3 class="text-emerald-400/60 text-[10px] font-black uppercase tracking-[0.32em] md:tracking-[0.4em] mb-9 md:mb-10 border-b border-emerald-300/20 pb-5">Support Protocol</h3>
                            ...
                            <div class="mb-10 md:mb-12 text-center lg:text-left">
                                <div class="flex items-baseline justify-center lg:justify-start gap-2 text-white mb-2">
                                    <span class="text-4xl sm:text-5xl xl:text-6xl font-black tracking-tight break-words" id="currentAmountDisplay">{{ number_format($campaign->current_amount) }} MAD</span>
                                </div>
                            ...
                            <div class="space-y-4 mb-10 md:mb-12">
                                <div class="w-full h-2.5 bg-black/40 rounded-full overflow-hidden p-0.5 border border-emerald-200/20">
                                    <div id="campaignProgressBar" class="h-full bg-gradient-to-r from-emerald-600 to-emerald-400 rounded-full transition-all duration-1000 shadow-[0_0_20px_rgba(52,211,153,0.3)]" style="width: {{ $pct }}%"></div>
                                </div>
                                <div class="flex justify-between items-center text-[10px] font-black uppercase text-white/50 tracking-[0.2em] px-1">
                                    <span id="campaignPercent" class="text-emerald-400">{{ number_format($pct, 1) }}%</span>
